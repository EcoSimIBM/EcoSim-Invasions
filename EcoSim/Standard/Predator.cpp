...
Predator::Predator(short type, long id, int species, long parent1ID, long parent2ID, int speciesParent1ID, int speciesParent2ID, Position pos, float speed, short age, float distanceEvol, FCMPredator *model, short genderVal, short isInvasive) : Sim(type, id, species, parent1ID, parent2ID, speciesParent1ID, speciesParent2ID, distanceEvol, nbGenes, genderVal, isInvasive) //Add Gender /MRE/
{
	setPosition(pos);
...
void Predator::reprod(Predator * friends, vector<Predator> * newPred, Ecosystem * eco) {
	...
	short isInvasive = this->getIsInvasive();
	...
	Predator * bebe = new Predator(bebe_type, bebe_id, bebe_species, bebe_parent1ID, bebe_parent2ID, bebe_speciesParent1ID, bebe_speciesParent2ID, bebe_position, bebe_speed, bebe_age, bebe_distanceEvol, &bebe_fcm, bebe_gender, isInvasive); //Add Gender /MRE/
	...
	if (friends->getIsInvasive() == 0) eco->statWorld.incAvgEnergyTransferredMalePred((friends->getStateBirth() * friends->getEnergyMax()) / 100);
	else eco->statWorld.incAvgEnergyTransferredInvMalePred((friends->getStateBirth() * friends->getEnergyMax()) / 100);
	if (this->getIsInvasive() == 0) eco->statWorld.incAvgEnergyTransferredFemalePred((this->getStateBirth() * this->getEnergyMax()) / 100);
	else eco->statWorld.incAvgEnergyTransferredInvFemalePred((this->getStateBirth() * this->getEnergyMax()) / 100);
...
void Predator::lookat(int nbList, int * distance, int * nbFriends, int * nbOpponents, int * nbFoods, Ecosystem * eco, Position * pTemp) {
	...
	if (*nbFriends < nbList) {
		...
		while (*nbFriends < nbList && int(nearbyFriends.size()) > 0) {
			...
			//if (this->getIsInvasive() == eco->wolves[nearbyFriends[fID]].getIsInvasive())
			//{
	...
	if (*nbFriends > 0)
	{
		...
		for (int cnt = 0; cnt < (int)nearbyFriends.size(); cnt++)
		{
			...
			if (eco->wolves[nearbyFriends[cnt]].getStrength() > StrengthStrongestPredatorClose && ((this->getGender() + 1) % 2) == eco->wolves[nearbyFriends[cnt]].getGender())// && this->getIsInvasive() == eco->wolves[nearbyFriends[cnt]].getIsInvasive())
			{
												...
												if (eco->rabbits[HuntList[k]].getLiving() && eco->rabbits[HuntList[k]].getEnergy() > 0) {
															...
															if (eco->rabbits[HuntList[k]].getID() != eco->rabbits[currcell_Preys[p]].getID() && eco->rabbits[currcell_Preys[p]].getLiving() && eco->rabbits[currcell_Preys[p]].getEnergy() > 0 && eco->rabbits[currcell_Preys[p]].getCoopDefence() > 0) {// not the same prey, only other prey in the same cell
																...
																if (eco->rabbits[currcell_Preys[p]].getEnergy() <= 0){
																	...
																	if (eco->rabbits[currcell_Preys[p]].getGender() == 1){
																		if (eco->rabbits[currcell_Preys[p]].getIsInvasive() == 0){
																			eco->getStat()->incAgeAvgDeadFemalePrey(eco->rabbits[currcell_Preys[p]].getAge());
																			eco->getStat()->incNbDeadFemalePreyK();
																		}else{
																			eco->getStat()->incAgeAvgDeadInvFemalePrey(eco->rabbits[currcell_Preys[p]].getAge());
																			eco->getStat()->incNbDeadInvFemalePreyK();
																		}
																	}
																	else{
																		if (eco->rabbits[currcell_Preys[p]].getIsInvasive() == 0){
																			...
																			eco->getStat()->incAgeAvgDeadInvMalePrey(eco->rabbits[currcell_Preys[p]].getAge());
																			eco->getStat()->incNbDeadInvMalePreyK();
																		}
																	}
																}
															}
													...
													if (prey_energy <= 0) {// Predator killed the Prey
														...
														if (eco->rabbits[HuntList[k]].getGender() == 1){
															if (eco->rabbits[HuntList[k]].getIsInvasive() == 0){
																eco->getStat()->incAgeAvgDeadFemalePrey(eco->rabbits[HuntList[k]].getAge());
																eco->getStat()->incNbDeadFemalePreyK();
															}else{
																eco->getStat()->incAgeAvgDeadInvFemalePrey(eco->rabbits[HuntList[k]].getAge());
																eco->getStat()->incNbDeadInvFemalePreyK();
															}
														}
														else{
															if (eco->rabbits[HuntList[k]].getIsInvasive() == 0){
																...
																eco->getStat()->incAgeAvgDeadInvMalePrey(eco->rabbits[HuntList[k]].getAge());
																eco->getStat()->incNbDeadInvMalePreyK();
															}
									 ...
									 if (getEnergy() < (this->getStateBirth() * this->getEnergyMax() / 100)) {
										 ...
										 if (MatingMode == 0){ //Random Mating
											 ...
											 while (pasFait && int(localMaleFriends.size()) > 0) {
												 ...
												 if (getID() != eco->wolves[localMaleFriends[k]].getID() && this->getIsInvasive() == eco->wolves[localMaleFriends[k]].getIsInvasive()) {    //-- Preventing for mating with itself
													 ...
													 if (eco->wolves[localMaleFriends[k]].getUpdated() == 0) { //partner has not acted
														 ...
														 if ((eco->wolves[localMaleFriends[k]]).getAge() >= (eco->wolves[localMaleFriends[k]]).getRepAge() && eco->wolves[localMaleFriends[k]].getFCM()->ReviseReproductionDecision(getPersuasion(), eco)){ //partner want to reproduce
															 ...
															 if (eco->wolves[localMaleFriends[k]].getEnergy() > (eco->wolves[localMaleFriends[k]].getEnergyMax() * eco->wolves[localMaleFriends[k]].getStateBirth() / 100)) { //-- Energy of partner is enough
																 ...
																 if (distance + model.calculDistanceEvol(eco, eco->wolves[localMaleFriends[k]].getFCM()) < eco->getDistanceSpeciesPred() || this->getIsInvasive() == 1) { //-- Distance is not far
																	 //DO REPRODUCTION
