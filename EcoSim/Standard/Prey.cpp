...
void Prey::reprod(Prey * friends, vector<Prey> * newPrey, Ecosystem * eco) {
	...
	Prey * bebe = new Prey(bebe_type, bebe_id, bebe_species, bebe_parent1ID, bebe_parent2ID, bebe_speciesParent1ID, bebe_speciesParent2ID, bebe_position, bebe_speed, bebe_age, bebe_distanceEvol, &bebe_fcm, bebe_gender, isInvasive);//Add Gender /MRE/
	...
	if (friends->getIsInvasive() == 0) eco->statWorld.incAvgEnergyTransferredMalePrey((friends->getStateBirth() * friends->getEnergyMax()) / 100);
	else  eco->statWorld.incAvgEnergyTransferredInvMalePrey((friends->getStateBirth() * friends->getEnergyMax()) / 100);
	if (this->getIsInvasive() == 0) eco->statWorld.incAvgEnergyTransferredFemalePrey((this->getStateBirth() * this->getEnergyMax()) / 100);
	else eco->statWorld.incAvgEnergyTransferredInvFemalePrey((this->getStateBirth() * this->getEnergyMax()) / 100);
...
void Prey::lookat(int nbList, int * distance, int * nbFriends, int * nbOpponents, int * nbFoods, Ecosystem * eco, Position * pTemp) {
	...
	if (*nbFriends < nbList) {
		...
		while (*nbFriends < nbList && int(nearbyFriends.size()) > 0) {
			...
			//if (this->getIsInvasive() == eco->rabbits[nearbyFriends[fID]].getIsInvasive())
			//{
	...
	if (*nbFriends > 0)
	{
		...
		for (int cnt = 0; cnt < (int)nearbyFriends.size(); cnt++)
		{
			...
			if (eco->rabbits[nearbyFriends[cnt]].getStrength() > getStrengthStrongestIndivClose() && eco->rabbits[nearbyFriends[cnt]].getGender() != this->getGender())// && eco->rabbits[nearbyFriends[cnt]].getIsInvasive() == this->getIsInvasive()) //Move2StrongestPrey
			{
								 ...
								 if (getEnergy() < (this->getStateBirth() * this->getEnergyMax() / 100)) {
									 ...
									 if (MatingMode == 0){ //Random Mating
										 ...
										 while (pasFait && int(LocalMaleFriends.size()) > 0) {
											 ...
											 if (getID() != eco->rabbits[LocalMaleFriends[k]].getID() && this->getIsInvasive() == eco->rabbits[LocalMaleFriends[k]].getIsInvasive()) { //-- Preventing for mating with itself
												 ...
												 if (eco->rabbits[LocalMaleFriends[k]].getUpdated() == 0) { //partner has not acted
													 ...
													 if ((eco->rabbits[LocalMaleFriends[k]]).getAge() >= (eco->rabbits[LocalMaleFriends[k]]).getRepAge() && eco->rabbits[LocalMaleFriends[k]].getFCM()->ReviseReproductionDecision(getPersuasion(), eco)){ //partner want to reproduce
														 ...
														 if (eco->rabbits[LocalMaleFriends[k]].getEnergy() > (eco->rabbits[LocalMaleFriends[k]].getEnergyMax() * eco->rabbits[LocalMaleFriends[k]].getStateBirth() / 100)) { //-- Energy of partner is enough
															 ...
															 if (distance + model.calculDistanceEvol(eco, eco->rabbits[LocalMaleFriends[k]].getFCM()) < eco->getDistanceSpeciesPrey() || this->getIsInvasive() == 1) { //-- Distance is not far
																 //DO REPRODUCTION
