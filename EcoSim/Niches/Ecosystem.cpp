...
Ecosystem::Ecosystem() {
	...
	invPreyPix.Blue=255; invPreyPix.Red=0; invPreyPix.Green=255; invPreyPix.Alpha=0;
	invPredPix.Blue=255; invPredPix.Red=255; invPredPix.Green=0; invPredPix.Alpha=0;
	...
	if (!paramMonde.is_open()) {		//-- Try to open parameters file
		...
		//invasions stuff starts here
		getline(paramMonde, entreInfo);
		getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);
		isTransferRun = atoi(words[1].c_str()); words.clear();		// is this run supposed to start normally but become an invader run? 1 = true, 0 = false
		getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);
		isAcceptor = atoi(words[1].c_str()); words.clear();			// is this run accepting invaders? 1 = true, 0 = false
		getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);
		isDonator = atoi(words[1].c_str()); words.clear();			// is this run donating invaders? 1 = true, 0 = false
		getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);
		numberPreyInvaders = atoi(words[1].c_str()); words.clear();		// how many prey invaders are we sending?
		getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);
		numberPredInvaders = atoi(words[1].c_str()); words.clear();		// how many pred invaders are we sending?
		getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);
		invasionFrequency = atoi(words[1].c_str()); words.clear();	// how often are we sending these invaders?
		getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);
		clonesOnly = atoi(words[1].c_str()); words.clear();	// are we only saving a clone or are we saving a varied population?
		...
		if (restore == 0) {
			...
			for (int i = 0; i < (initNbPrey / perClusterPrey); ++i) {		//-- Creates Prey
				...
				for (int j = 1; j <= perClusterPrey; j++) {
					...
					{
						newPrey = new Prey(type, ID, species, Parent1ID, Parent2ID, SpeciesParent1ID, SpeciesParent2ID, position, energy, energyMaxPrey, speed, speedMaxPrey, age, ageMax, vsionPrey, ageReprodPrey, DistEvolInitial, stateBirthPreyFemale, &fcm_prey, genderVal, 0, 0.05, 0.05); //Add Gender /MRE/
					}
					else
					{ //baby is male
						newPrey = new Prey(type, ID, species, Parent1ID, Parent2ID, SpeciesParent1ID, SpeciesParent2ID, position, energy, energyMaxPrey, speed, speedMaxPrey, age, ageMax, vsionPrey, ageReprodPrey, DistEvolInitial, stateBirthPreyMale, &fcm_prey, genderVal, 0, 0.05, 0.05); //Add Gender /MRE/
					}
			...
			for (int i = 0; i < (initNbPredator / perClusterPredator); ++i) {	//-- Creates Predator
				...
				for (int j = 1; j <= perClusterPredator; ++j) {
					...
					if (genderVal == 1)//1 means Gender::Female
					{
						newPred = new Predator(type, ID, species, Parent1ID, Parent2ID, SpeciesParent1ID, SpeciesParent2ID, position, energy, energyMaxPredator, speed, speedMaxPredator, age, ageMax, visionPredator, ageReprodPredator, DistEvolInitial, stateBirthPredFemale, &fcm_pred, genderVal, 0); //Add Gender /MRE/Error
					}
					else{ //baby is male
						newPred = new Predator(type, ID, species, Parent1ID, Parent2ID, SpeciesParent1ID, SpeciesParent2ID, position, energy, energyMaxPredator, speed, speedMaxPredator, age, ageMax, visionPredator, ageReprodPredator, DistEvolInitial, stateBirthPredMale, &fcm_pred, genderVal, 0); //Add Gender /MRE/Error
					}
...
int Ecosystem::getNbLocalSameType(Position p, int type, int isInvasive) {
	
	vector<int> list = *(getListCreature(p, type));
	int i;
	int nb = 0;
	...
	{
		if(type == 0){
			if (rabbits[list[i]].getIsInvasive() == isInvasive) nb++;
		}else{
			if (wolves[list[i]].getIsInvasive() == isInvasive) nb++;
		}
	}
	return nb;
}
...
void Ecosystem::updateEco() {
	...
	*/
	...
	for (unsigned int i = 0; i < rabbits.size(); i++) { // Act based on strength M.M; who has more strength act first
		...
		if (rabbits[i].getLastAction() != 0) {
			if (rabbits[i].getGender() == 0) //male
			{
				if (rabbits[i].getIsInvasive() == 0){
					statWorld.incNbActionPreyMale(rabbits[i].getLastAction() - 1);
				}
				else{
					statWorld.incNbActionInvPreyMale(rabbits[i].getLastAction() - 1);
				}
			}
			else
			{
				if (rabbits[i].getIsInvasive() == 0){
					...
					statWorld.incNbActionInvPreyFemale(rabbits[i].getLastAction() - 1);
				}
			}
			if (rabbits[i].getLastAction() == FCMPrey::ReproduceAction)
			{
				if (rabbits[i].getIsInvasive() == 0){
					...
					statWorld.incAvgDistMatingInvPrey(rabbits[i].get_DistMating());
				}
			}
		}
		if (rabbits[i].getEnergy() <= 0 || rabbits[i].getAge() > rabbits[i].getAgeMax(true)) {
			...
			if (rabbits[i].getGender() == 1)
			{
				if (rabbits[i].getIsInvasive() == 0){
					...
					statWorld.incAgeAvgDeadInvFemalePrey(rabbits[i].getAge());
				}
			}
			else
			{
				if (rabbits[i].getIsInvasive() == 0){
					...
					statWorld.incAgeAvgDeadInvMalePrey(rabbits[i].getAge());
				}
			}
			...
			if (rabbits[i].getEnergy() <= 0) {
				if (rabbits[i].getGender() == 1)
				{
					if (rabbits[i].getIsInvasive() == 0){
						...
						statWorld.incNbDeadInvFemalePreyE();
					}
				}
				else
				{
					if (rabbits[i].getIsInvasive() == 0){
						...
						statWorld.incNbDeadInvMalePreyE();
					}
				}
				...
				if (rabbits[i].getGender() == 1)
				{
					if (rabbits[i].getIsInvasive() == 0){
						...
						statWorld.incNbDeadInvFemalePreyA();
					}
				}
				else
				{
					if (rabbits[i].getIsInvasive() == 0){
						...
						statWorld.incNbDeadInvMalePreyA();
					}
				}
	...
	for (size_t i = 0; i < rabbits.size(); ++i) {
		if (rabbits[i].getGender() == 1){
			if (rabbits[i].getIsInvasive() == 0) statWorld.incSpeedAvgFemalePrey(rabbits[i].getSpeed());
			else statWorld.incSpeedAvgInvFemalePrey(rabbits[i].getSpeed());
		}
		else
		{
			if (rabbits[i].getIsInvasive() == 0) statWorld.incSpeedAvgMalePrey(rabbits[i].getSpeed());
			else statWorld.incSpeedAvgInvMalePrey(rabbits[i].getSpeed());
		}
	}
	...
	for (size_t i = 0; i < rabbits.size(); ++i) {
		if (rabbits[i].getGender() == 1){
			...
			for (size_t j = 0; j < temp->get_rows(); ++j) {
				if (rabbits[i].getIsInvasive() == 0) statWorld.inc_activation_prey_female(j, (*temp->get_activations())[j]);
				else statWorld.inc_activation_Invprey_female(j, (*temp->get_activations())[j]);
			}
		}
		else{
			FCM * temp = rabbits[i].getFCM();
			for (size_t j = 0; j < temp->get_rows(); ++j) {
				if (rabbits[i].getIsInvasive() == 0) statWorld.inc_activation_prey_male(j, (*temp->get_activations())[j]);
				else statWorld.inc_activation_Invprey_male(j, (*temp->get_activations())[j]);
			}
		}
	}
	/*
	...
	*/
	...
	for (size_t i = 0; i < newPrey.size(); ++i) {
		...
		{
			if (newPrey[i].getIsInvasive() == 0) statWorld.incNbBirthFemalePrey();
			else statWorld.incNbBirthInvFemalePrey();
		}
		else
		{
			if (newPrey[i].getIsInvasive() == 0) statWorld.incNbBirthMalePrey();
			else statWorld.incNbBirthInvMalePrey();
		}
	...
	*/
	...
	for (unsigned int i = 0; i< wolves.size(); i++) { // Act based on strength  M.M
		...
		if (wolves[i].getLastAction() != 0) {
			if (wolves[i].getGender() == 0) //male
			{
				if (wolves[i].getIsInvasive() == 0) statWorld.incNbActionPredMale(wolves[i].getLastAction() - 1);
				else statWorld.incNbActionInvPredMale(wolves[i].getLastAction() - 1);
			}
			else
			{
				if (wolves[i].getIsInvasive() == 0) statWorld.incNbActionPredFemale(wolves[i].getLastAction() - 1);
				else statWorld.incNbActionInvPredFemale(wolves[i].getLastAction() - 1);
			}
			if (wolves[i].getLastAction() == FCMPredator::ReproduceAction)
			{
				if (wolves[i].getIsInvasive() == 0) statWorld.incAvgDistMatingPred(wolves[i].get_DistMating());  //-- Meisam
				else statWorld.incAvgDistMatingInvPred(wolves[i].get_DistMating());
			}
		}
		...
		if (wolves[i].getEnergy() <= 0 || wolves[i].getAge() > wolves[i].getAgeMax(true)) {
			...
			{
				if (wolves[i].getIsInvasive() == 0) statWorld.incAgeAvgDeadFemalePred(wolves[i].getAge());
				else statWorld.incAgeAvgDeadInvFemalePred(wolves[i].getAge());
			}
			else
			{
				if (wolves[i].getIsInvasive() == 0) statWorld.incAgeAvgDeadMalePred(wolves[i].getAge());
				else statWorld.incAgeAvgDeadInvMalePred(wolves[i].getAge());
			}
			...
			if (wolves[i].getEnergy() <= 0) {
				if (wolves[i].getKilledInFight()){ // M.M
					if (wolves[i].getGender() == 1)
					{
						if (wolves[i].getIsInvasive() == 0) statWorld.incnbDeadFemalePredF();	  // M.M
						else statWorld.incnbDeadInvFemalePredF();	  // M.M
					}
					else
					{
						if (wolves[i].getIsInvasive() == 0) statWorld.incnbDeadMalePredF();	  // M.M
						else statWorld.incnbDeadInvMalePredF();	  // M.M
					}
					...
					if (wolves[i].getGender() == 1)
					{
						if (wolves[i].getIsInvasive() == 0) statWorld.incNbDeadFemalePredE();	  // M.M
						else statWorld.incNbDeadInvFemalePredE();	  // M.M
					}
					else
					{
						if (wolves[i].getIsInvasive() == 0) statWorld.incNbDeadMalePredE();
						else statWorld.incNbDeadInvMalePredE();
					}
				...
				if (wolves[i].getGender() == 1)
				{
					if (wolves[i].getIsInvasive() == 0) statWorld.incNbDeadFemalePredA();	  // M.M
					else statWorld.incNbDeadInvFemalePredA();	  // M.M
				}
				else
				{
					if (wolves[i].getIsInvasive() == 0) statWorld.incNbDeadMalePredA();
					else statWorld.incNbDeadInvMalePredA();
				}
	...
	for (size_t i = 0; i < wolves.size(); ++i) {
		if (wolves[i].getGender() == 1)
		{
			if (wolves[i].getIsInvasive() == 0) statWorld.incSpeedAvgFemalePred(wolves[i].getSpeed());
			else statWorld.incSpeedAvgInvFemalePred(wolves[i].getSpeed());
		}
		else
		{
			if (wolves[i].getIsInvasive() == 0) statWorld.incSpeedAvgMalePred(wolves[i].getSpeed());
			else statWorld.incSpeedAvgInvMalePred(wolves[i].getSpeed());			
		}
	}
	...
	for (size_t i = 0; i < wolves.size(); ++i) {
		if (wolves[i].getGender() == 1){
			...
			for (size_t j = 0; j < temp->get_rows(); ++j) {
				if (wolves[i].getIsInvasive() == 0) statWorld.inc_activation_pred_female(j, (*temp->get_activations())[j]);
				else statWorld.inc_activation_Invpred_female(j, (*temp->get_activations())[j]);
			}
		}
		else{
			FCM * temp = wolves[i].getFCM();
			for (size_t j = 0; j < temp->get_rows(); ++j) {
				if (wolves[i].getIsInvasive() == 0) statWorld.inc_activation_pred_male(j, (*temp->get_activations())[j]);
				else statWorld.inc_activation_Invpred_male(j, (*temp->get_activations())[j]);
			}
		}
	}
	...
	*/
	...
	for (size_t i = 0; i < newPredator.size(); ++i) {
		...
		{
			if (newPredator[i].getIsInvasive() == 0) statWorld.incNbBirthFemalePred();
			else statWorld.incNbBirthInvFemalePred();
		}
		else
		{
			if (newPredator[i].getIsInvasive() == 0) statWorld.incNbBirthMalePred();
			else statWorld.incNbBirthInvMalePred();
		}
	...
	*/
	...
	for (size_t i = 0; i < rabbits.size(); ++i) {
		...
		{
			if (rabbits[i].getGender() == 1){
				statWorld.incNbInvFemalePrey();
				statWorld.incNbArcAvgInvFemalePrey(rabbits[i].getFCM()->getNbArc());
				statWorld.incAgeInvFemalePrey(rabbits[i].getAge());
				statWorld.incEnergyAvgInvFemalePrey(rabbits[i].getEnergy());
				statWorld.incStrengthAvgInvFemalePrey(rabbits[i].getStrength()); // M.M
				statWorld.incStateAvgInvFemalePrey(rabbits[i].getStateBirth());
				statWorld.incAvgDistEvInvFemalePrey(rabbits[i].getDistanceEvol());
				statWorld.incPersuasionInvFemalePrey(rabbits[i].getPersuasion());
			}
			else{
				statWorld.incNbInvMalePrey();
				statWorld.incNbArcAvgInvMalePrey(rabbits[i].getFCM()->getNbArc());
				statWorld.incAgeInvMalePrey(rabbits[i].getAge());
				statWorld.incEnergyAvgInvMalePrey(rabbits[i].getEnergy());
				statWorld.incStrengthAvgInvMalePrey(rabbits[i].getStrength()); // M.M
				statWorld.incStateAvgInvMalePrey(rabbits[i].getStateBirth());
				statWorld.incAvgDistEvInvMalePrey(rabbits[i].getDistanceEvol());
				statWorld.incPersuasionInvMalePrey(rabbits[i].getPersuasion());
			}
	...
	for (size_t i = 0; i < wolves.size(); ++i) {
		...
		{
			if (wolves[i].getGender() == 1){
				statWorld.incNbInvFemalePred();
				statWorld.incNbArcAvgInvFemalePred(wolves[i].getFCM()->getNbArc());
				statWorld.incAgeInvFemalePred(wolves[i].getAge());
				statWorld.incEnergyAvgInvFemalePred(wolves[i].getEnergy());
				statWorld.incStrengthAvgInvFemalePred(wolves[i].getStrength()); // M.M
				statWorld.incStateAvgInvFemalePred(wolves[i].getStateBirth());
				statWorld.incAvgDistEvInvFemalePred(wolves[i].getDistanceEvol());
				statWorld.incPersuasionInvFemalePred(wolves[i].getPersuasion());
			}
			else{
				statWorld.incNbInvMalePred();
				statWorld.incNbArcAvgInvMalePred(wolves[i].getFCM()->getNbArc());
				statWorld.incAgeInvMalePred(wolves[i].getAge());
				statWorld.incEnergyAvgInvMalePred(wolves[i].getEnergy());
				statWorld.incStrengthAvgInvMalePred(wolves[i].getStrength()); // M.M
				statWorld.incStateAvgInvMalePred(wolves[i].getStateBirth());
				statWorld.incAvgDistEvInvMalePred(wolves[i].getDistanceEvol());
				statWorld.incPersuasionInvMalePred(wolves[i].getPersuasion());
			}
		}
	}
	...
	for (int i = 0; i < width; ++i) {
		...
		for (int j = 0; j < height; ++j) {
			...
			if ((int)getListCreature(position, 0)->size() > 0) {
				...
				statWorld.incNbInvPreyByCase((int)getListCreature(position, 0)->size());
				statWorld.incNbCaseInvPrey();
			}
			if ((int)getListCreature(position, 1)->size() > 0) {
				...
				statWorld.incNbInvPredByCase((int)getListCreature(position, 1)->size());
				statWorld.incNbCaseInvPred();
			}*/
			if ((int)getNbLocalSameType(position, 0, 0) > 0) {//we have native prey
				statWorld.incNbPreyByCase((int)getNbLocalSameType(position, 0, 0));
				statWorld.incNbCasePrey();
			}
			
			if ((int)getNbLocalSameType(position, 0, 1) > 0) {//we have nonnative prey
				statWorld.incNbInvPreyByCase((int)getNbLocalSameType(position, 0, 1));
				statWorld.incNbCaseInvPrey();
			}
			
			if ((int)getNbLocalSameType(position, 1, 0) > 0) {//we have native preds
				statWorld.incNbPredByCase((int)getNbLocalSameType(position, 1, 0));
				statWorld.incNbCasePred();
			}
			
			if ((int)getNbLocalSameType(position, 1, 1) > 0) {//we have nonnative preds
				statWorld.incNbInvPredByCase((int)getNbLocalSameType(position, 1, 1));
				statWorld.incNbCaseInvPred();
			}
		}
	}
	...
	*/
	...
	cout << "\nStats..." << endl; cout.flush();
	SpeciesStatistics(); //-- Meisam: Just for Results file
	
	cout << "\nCreating an image..." << endl; cout.flush();
	snap();
}
...
void Ecosystem::minSave() {
	...
	for (i = 0; i < rabbits.size(); i++) {
		...
		if (rabbits[i].getID() <= PrevRabbitsLastID) {
			...
			buffer += Manip.vtos((short)rabbits[i].getIsInvasive()) + "\n";
	...
	for (i = 0; i < wolves.size(); i++) {
		if (wolves[i].getID() <= PrevWolvesLastID){
			...
			buffer += Manip.vtos((short)wolves[i].getIsInvasive()) + "\n";
...
void Ecosystem::maxSave() {
	...
	for (size_t i = 0; i < rabbits.size(); i++) {
		...
		buffer += "20: " + Manip.vtos((short)rabbits[i].getIsInvasive()) + "\n";
		stateFile << buffer;
		buffer.clear();
	...
	for (size_t i = 0; i < wolves.size(); i++) {
		...
		buffer += "20: " + Manip.vtos((short)wolves[i].getIsInvasive()) + "\n";
		stateFile << buffer;
		buffer.clear();
	...
	buffer.clear();
	stateFile << "\n" << "PreyInvasiveSpec: " << Manip.vtos((int)preySpeciesFound.size()) << endl;
	int f = 0;
	for (f = 0; f < preySpeciesFound.size(); f++) {
		stateFile << Manip.vtos(preySpeciesFound[f]) << endl;
	}
	buffer.clear();

	stateFile << "\n" << "PredInvasiveSpec: " << (int)predSpeciesFound.size() << endl;
	for (f = 0; f < predSpeciesFound.size(); f++) {
		stateFile << Manip.vtos(predSpeciesFound[f]) << endl;
	}

	buffer.clear();
	stateFile << "\n" << "PreyInvasiveSpecMappedTo: " << Manip.vtos((int)preySpeciesMapped.size()) << endl;
	for (f = 0; f < preySpeciesMapped.size(); f++) {
		stateFile << Manip.vtos(preySpeciesMapped[f]) << endl;
	}

	buffer.clear();
	stateFile << "\n" << "PredInvasiveSpecMappedTo: " << (int)predSpeciesMapped.size() << endl;
	for (f = 0; f < predSpeciesMapped.size(); f++) {
		stateFile << Manip.vtos(predSpeciesMapped[f]) << endl;
	}
...
/*
void Ecosystem::saveInvaders() { 	// Break into Preds and Prey?
									// Further, randomly select individuals by species etc?
	//TODO: below randomly selects k prey and j preds... what if I want to just select clones?
	int gen = generation;
	
	//int isTransferRun;		//Need to use in MaxSave function
	//int isAcceptor;			//Done - EcoSim.cpp
	//int isDonator;			//Done - EcoSim.cpp
	//int numberPreyInvaders;		//Done in saveInvaders - take a look at loadInvaders
	//int numberPredInvaders;		//Done in saveInvaders - take a look at loadInvaders
	//int invasionFrequency;  //Done - EcoSim.cpp
	//int clonesOnly;			//Done in saveInvaders - loadInvaders should work the same regardless
	
	cout << " Saving Invaders" << flush;

	time(&start);

	//-- Open file for writing
	ostringstream o;
	o << "InvadersSave" << Manip.vtos(gen) << ".txt";
	string s = o.str();
	ofstream invSave;
	invSave.open(s.c_str(), ios::out | ios::trunc | ios::binary);
	...
	printf("Copying prey and predator vectors");
	vector <Prey> invasivePrey;
	vector <Predator> invasivePred;
	vector <Prey> preyCopy (rabbits);
	vector <Predator> predCopy (wolves);
	
	printf("Selecting invasives");
	int theChosenOnePrey;
	int theChosenOnePred;
	if (clonesOnly == 0){ //Get k random prey and k random preds
		while (invasivePrey.size() < numberPreyInvaders){
			int randizzle = rand.next(0, preyCopy.size()-1);
			invasivePrey.push_back(preyCopy[randizzle]);
			preyCopy.erase(preyCopy.begin() + randizzle);
		} //at this point, preyCopy only has numberPreyInvaders elements (unless there were not enough)
		
		while (invasivePred.size() < numberPredInvaders){
			int randizzle = rand.next(0, predCopy.size()-1);
			invasivePred.push_back(predCopy[randizzle]);
			predCopy.erase(predCopy.begin() + randizzle);
		} //at this point, predCopy only has numberPredInvaders elements (unless there were not enough)
		
		if (invasivePrey.size() != numberPreyInvaders){
			printf("INVADERS - incorrect number of prey for transfer! See SaveInvaders in Ecosystem.cpp!");
		}
		else if (invasivePred.size() != numberPredInvaders){
			printf("INVADERS - incorrect number of preds for transfer! See SaveInvaders in Ecosystem.cpp!");
		}
	}
	else{ //Get a single prey and pred
		while (invasivePrey.size() < numberPreyInvaders){
			int randizzle = rand.next(0, preyCopy.size()-1);
			invasivePrey.push_back(preyCopy[randizzle]);
			preyCopy.erase(preyCopy.begin() + randizzle);
		...
		theChosenOnePrey = rand.next(0, invasivePrey.size()-1);
		theChosenOnePred = rand.next(0, invasivePred.size()-1);
	}

	
	printf("Writing invasives");
	//-- Prey
	invSave << "Prey: " << Manip.vtos(invasivePrey.size()) << endl;
	
	for (int i = 0; i < invasivePrey.size(); i++) {
		int numb;
		if (clonesOnly == 0) numb = i;
		else numb = theChosenOnePrey;
		//-- Details
		buffer += "1: " + Manip.vtos(invasivePrey[numb].getPosition()->x) + " " + Manip.vtos(invasivePrey[numb].getPosition()->y) + "\n";//yes
		buffer += "2: " + Manip.vtos(invasivePrey[i].getEnergy()) + "\n"; //yes
		buffer += "3: " + Manip.vtos(invasivePrey[numb].getEnergyMax()) + "\n"; //yes
		buffer += "4: " + Manip.vtos(invasivePrey[i].getAge()) + "\n";	//yes
		buffer += "5: " + Manip.vtos(invasivePrey[numb].getAgeMax(false)) + "\n"; //yes
		buffer += "6: " + Manip.vtos(invasivePrey[numb].getStateBirth()) + "\n"; //yes
		buffer += "7: " + Manip.vtos(invasivePrey[numb].getSpeed()) + "\n"; //yes
		buffer += "8: " + Manip.vtos(invasivePrey[numb].getDistanceEvol()) + "\n"; //yes
		buffer += "9: " + Manip.vtos(invasivePrey[numb].getLastAction()) + "\n"; //no... don't need anyway
		buffer += "10: " + Manip.vtos(invasivePrey[numb].get_action_offset()) + "\n"; //no... don't need anyway
		buffer += "11: " + Manip.vtos(invasivePrey[numb].getSpecies()) + "\n"; //yes
		buffer += "12: " + Manip.vtos(invasivePrey[numb].getSpeedMax()) + "\n"; //yes
		buffer += "13: " + Manip.vtos(invasivePrey[numb].getVision()) + "\n"; //yes
		buffer += "14: " + Manip.vtos(invasivePrey[numb].getSpeciesParent1ID()) + "\n"; //yes
		buffer += "15: " + Manip.vtos(invasivePrey[numb].getSpeciesParent2ID()) + "\n"; //yes
		buffer += "16: " + Manip.vtos(invasivePrey[numb].getParent1ID()) + "\n"; //yes
		buffer += "17: " + Manip.vtos(invasivePrey[numb].getParent2ID()) + "\n"; //yes
		buffer += "18: " + Manip.vtos(invasivePrey[i].getGender()) + "\n";//Add Gender /MRE/ //yes
		buffer += "19: " + Manip.vtos((short)invasivePrey[numb].getPersuasionChar()) + "\n"; //yes
		invSave << buffer;
		buffer.clear();
		...
		buffer.clear();
		invSave << "" << endl;
		fTemp = invasivePrey[numb].getFCM();
		for (size_t j = 0; j < fTemp->get_rows(); j++) {
			buffer += Manip.vtos(fTemp->get_activation(j)) + " ";
		}
		...
		//-- Physical Genome
		for (size_t j = 0; j < Prey::nbGenes; j++)					      //M.M
			buffer += Manip.vtos(invasivePrey[numb].getPhGenome()->getGene(j)) + " "; // M.M  real values of genes
		buffer += "\n";
		invSave << buffer;  // M.M
		buffer.clear();		// M.M
	}
	...
	//-- Pred
	invSave << "\n" << "Pred: " << Manip.vtos(invasivePred.size()) << endl;
	for (int i = 0; i < invasivePred.size(); i++) {
		int numb;
		if (clonesOnly == 0) numb = i;
		else numb = theChosenOnePred;
		//-- Details
		buffer += "1: " + Manip.vtos(invasivePred[numb].getPosition()->x) + " " + Manip.vtos(invasivePred[numb].getPosition()->y) + "\n";
		
		if (clonesOnly == 0) buffer += "2: " + Manip.vtos(invasivePred[numb].getEnergy()) + "\n";	//Energy should not be cloned... it should be drawn from same distribution as non-clones
		else buffer += "2: " + Manip.vtos(invasivePred[i].getEnergy()) + "\n";
		buffer += "3: " + Manip.vtos(invasivePred[numb].getEnergyMax()) + "\n";
		if (clonesOnly == 0) buffer += "4: " + Manip.vtos(invasivePred[numb].getAge()) + "\n";		//Age should not be cloned... it should be drawn from same distribution as non-clones
		else buffer += "4: " + Manip.vtos(invasivePred[i].getAge()) + "\n";	
		buffer += "5: " + Manip.vtos(invasivePred[numb].getAgeMax(false)) + "\n";
		buffer += "6: " + Manip.vtos(invasivePred[numb].getStateBirth()) + "\n";
		buffer += "7: " + Manip.vtos(invasivePred[numb].getSpeed()) + "\n";
		buffer += "8: " + Manip.vtos(invasivePred[numb].getDistanceEvol()) + "\n";
		buffer += "9: " + Manip.vtos(invasivePred[numb].getLastAction()) + "\n";
		buffer += "10: " + Manip.vtos(invasivePred[numb].get_action_offset()) + "\n";
		buffer += "11: " + Manip.vtos(invasivePred[numb].getSpecies()) + "\n";
		buffer += "12: " + Manip.vtos(invasivePred[numb].getSpeedMax()) + "\n";
		buffer += "13: " + Manip.vtos(invasivePred[numb].getVision()) + "\n";
		buffer += "14: " + Manip.vtos(invasivePred[numb].getSpeciesParent1ID()) + "\n";
		buffer += "15: " + Manip.vtos(invasivePred[numb].getSpeciesParent2ID()) + "\n";
		buffer += "16: " + Manip.vtos(invasivePred[numb].getParent1ID()) + "\n";
		buffer += "17: " + Manip.vtos(invasivePred[numb].getParent2ID()) + "\n";
		if (clonesOnly == 0) buffer += "18: " + Manip.vtos(invasivePred[numb].getGender()) + "\n";//Add Gender /MRE/
		else buffer += "18: " + Manip.vtos(rand.next(0, 1)) + "\n";//Add Gender /MRE/ randomize gender if getting clones
		buffer += "19: " + Manip.vtos((short)invasivePred[numb].getPersuasionChar()) + "\n";
		invSave << buffer;
		buffer.clear();

		//-- Concepts and concepts Old
		invSave << "" << endl;
		fTemp = invasivePred[numb].getFCM();
		buffer.clear();
		for (size_t j = 0; j < fTemp->get_rows(); j++) {
			buffer += Manip.vtos(fTemp->get_activation(j)) + " ";
		}
		...
		//-- genome
		for (size_t j = 0; j < Predator::nbGenes; j++)			//M.M
			buffer += Manip.vtos(invasivePred[numb].getPhGenome()->getGene(j)) + " ";
		buffer += "\n";
		invSave << buffer;  // M.M
		buffer.clear();		// M.M
	}

	//-- Species //Just going to save the species like this, but will do the tricky stuff on load
	buffer.clear();
	invSave << "\n" << "PreySpec: " << Manip.vtos((int)speciesPrey.size()) << endl;
	for (list<Species>::iterator S = speciesPrey.begin(); S != speciesPrey.end(); ++S) {
		invSave << Manip.vtos((*S).getNum()) << " " << Manip.vtos((*S).getParent()) << endl;
	}
	buffer.clear();
	invSave << "\n" << "PredSpec: " << (int)speciesPred.size() << endl;
	for (list<Species>::iterator S = speciesPred.begin(); S != speciesPred.end(); ++S) {
...
void Ecosystem::loadInvaders() { //Going to have to do this somewhat manually in terms of file name
	...
	int gen = generation;
	char fname[50];
	sprintf(fname, "InvadersSave%i.txt", gen);

	ifstream paramMonde(fname);
	
	
	if (!paramMonde.is_open()) {
		cout << "Error! InvadersSave not found" << endl;
		exit(-1);
	}
	
	//-- Details of the individuals
	bool DEBUGRESTORE = false;
	long iid;
	...
	float killed; //just use this to kill off irrelevant information
	short genderVal;	//Add Gender /MRE/ Female=1, Male=0
	short persuasionVal;
	...
	cout << "Loading Invaders " << generation << endl;
	...
	//-- Restore the prey
	cout << " - Loading " << numPrey << " invasive prey." << endl;
	ofstream invMapping;
	invMapping.open("invMapping.csv", ios::app);
	
	int newSpeciesCreated = 0;
	//This loop previously iterated up to numPrey but I changed it to numberPreyInvaders instead
	for (int i = 0; i < numberPreyInvaders; i++) {  //This loop is fine, it's just going to create new individuals but instantly add them
		...
		words.clear();
		getline(paramMonde, entreInfo); Manip.Tokenize(entreInfo, words);
		iid = (long)atol(words[1].c_str());  words.clear();
		getline(paramMonde, entreInfo); Manip.Tokenize(entreInfo, words); //-- Position
		//need to decide the positioning ourselves... not match the other EcoSim's positioning
		p.x = rand.next(495, 505);
		p.y = rand.next(495, 505);  words.clear();
		...
		killed = (float)atof(words[1].c_str());  words.clear();
		...
		killed = atoi(words[1].c_str());  words.clear();
		...
		spec = -2; //atoi(words[1].c_str());  words.clear();
		...
		int prey_genome[Prey::nbGenes];  // M.M
		getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);  // M.M
		//getting the physical genome as floats.. need it as ints
		for (size_t j = 0; j < Prey::nbGenes; j++)   	  // M.M
			prey_genome[j] = atoi(words[j].c_str());   // M.M
		short aMO = (short)(0); // ageMaxThreshold
		//energy max needs change
		eM = char2Float(prey_genome[0], 100, 25);	// M.M
		aM = (short)prey_genome[1];	// M.M
		v = (short)prey_genome[2]; 	// M.M
		sM = (short)prey_genome[3]; 	// M.M
		rA = (short)prey_genome[4]; 	// M.M
		//sob needs change
		sB = char2Float(prey_genome[5], 0, 0.1);				// M.M
		//defense needs change
		d = char2Float(prey_genome[6], 0, 1.0/256.0);				// M.M
		//coop defense needs change
		cd = char2Float(prey_genome[7], 0, 1.0/512.0);			// M.M
		
		fcm_prey.ConceptIndex = ConceptIndex;           //-- Meisam
		fcm_prey.ConceptMultiplier = ConceptMultiplier; //-- Meisam
		if (DEBUGRESTORE) printf("\nRestored prey physical genome as shorts: %d %d %d %d %d %d %d %d\n", prey_genome[0], prey_genome[1], prey_genome[2], prey_genome[3], prey_genome[4], prey_genome[5], prey_genome[6], prey_genome[7]);
		if (DEBUGRESTORE) printf("\nRestored prey physical genome as floats: %f %d %d %d %d %f %f %f\n", eM, aM, v, sM, rA, sB, d, cd);
		cout.flush();
		Prey * newPrey = new Prey(t, maxPreyID, spec, p1id, p2id, p1sid, p2sid, p, e, eM, spd, sM, a, aM, v, rA, dE, sB, &fcm_prey, genderVal, 1, d, cd, persuasionVal);	//Add Gender /MRE/
		//cout << Manip.vtos(t) << "," << Manip.vtos(maxPreyID) << "," << Manip.vtos(spec) << ","  << Manip.vtos(e) << "," << Manip.vtos(eM) << "," << Manip.vtos(spd) << "," << Manip.vtos(sM) << "," << Manip.vtos(a) << "," << Manip.vtos(aM) << "," << Manip.vtos(v) << "," << Manip.vtos(rA) << "," << Manip.vtos(dE) << "," << Manip.vtos(sB) << "," << Manip.vtos(genderVal) << "," << Manip.vtos(d) << "," << Manip.vtos(cd) << endl;
		//cout.flush();
		
		invMapping << iid << " " << maxPreyID << "\n";
		maxPreyID++;
		...
		int speciesIndex = 0;
		bool foundTheSpecies = false;
		
		
		for (speciesIndex = 0; speciesIndex < preySpeciesFound.size(); speciesIndex++)
		{
			if (preySpeciesFound[speciesIndex] == spec) //found the species
			{
				foundTheSpecies = true;
				//cout << "\nFOUND THE SPECIES!!! " << spec << endl; cout.flush();
				break;
			} 
		}
		//what if we have the mapping but the species hasn't been recreated yet?
		//species mapping exists because the species was there... species in this instance died off, species is removed
		//species mapping still exists but actual species does not!
			//it's not an error, it's an actual case that can occur!
		
		if (foundTheSpecies)
		{
			bool allocated = false;
			int speciesID = preySpeciesMapped[speciesIndex];
			for (list<Species>::iterator S = speciesPrey.begin(); S != speciesPrey.end(); ++S) //find the species
			{
				if ((*S).getNum() == speciesID) //found it
				{
					(*S).preyMembers.push_back(&(rabbits.back())); //add this prey to preyMembers of that species
					(rabbits.back()).setSpecies((*S).getNum()); //set the species of the prey to the correct species
					(rabbits.back()).speciesPtr = &(*S); //update its pointer
					(*S).incNbMembers();
					allocated = true;
			...
			if (allocated == false)
			{
				printf("\ndidn't find the prey species %d somehow though we determined that it exists as %d... need to re-create it\n", spec, speciesID);

				cout << "\nThe invasive prey species " << spec << " hasn't been made yet in this instance though it existed before... creating new species with ID " << speciesID << endl; cout.flush();
				FCMPrey s_fcm_prey;
				s_fcm_prey = FCMPrey((rabbits.back()).getFCM()->get_chart(), NULL, &(rabbits.back()));
				PhysicalGenome* genome_temp = (rabbits.back()).getPhGenome();	// M.M
				...
				Species * newPreySpecies = new Species(-2, 0, &s_fcm_prey, &s_genome_prey); // M.M modified
				speciesPrey.push_back(*newPreySpecies);
				speciesPrey.back().preyMembers.clear();
				speciesPrey.back().preyMembers.push_back(&(rabbits.back()));
				rabbits.back().speciesPtr = &(speciesPrey.back());
				delete newPreySpecies;
				newSpeciesCreated++;
				//preySpeciesFound.push_back(spec);
				//preySpeciesMapped.push_back(Species::numSpeciesPreyMax);
				speciesPrey.back().incNbMembers();
				speciesPrey.back().setParent(-2);
			}
		}
		else
		{
			//Species::numSpeciesPreyMax++;
			cout << "\nThe invasive prey species " << spec << " hasn't been made yet... creating new species with ID " << Species::numSpeciesPreyMax << endl; cout.flush();
			FCMPrey s_fcm_prey;
			s_fcm_prey = FCMPrey((rabbits.back()).getFCM()->get_chart(), NULL, &(rabbits.back()));
			PhysicalGenome* genome_temp = (rabbits.back()).getPhGenome();	// M.M
			//PhysicalGenome s_genome_prey = PhysicalGenome (Prey::nbGenes, genome_temp->getGenome(), probaMutation); // M.M
			...
			newSpeciesCreated++;
			preySpeciesFound.push_back(spec);
			preySpeciesMapped.push_back(-2);
			speciesPrey.back().incNbMembers();
			speciesPrey.back().setParent(-2);
			//create a new species and add this individual to that
				//add a value to preySpeciesFound and its corresponding species ID in preySpeciesMapped
		}
	...
	cout << "   " << numPrey << " prey, " << newSpeciesCreated << " invasive species created." << endl;
	
	/*
	// RESTORE PREDATOR
	
	getline(paramMonde, entreInfo); words.clear();
	...
	//-- Restore the pred
	cout << " - Loading " << numPred << " invasive preds." << endl;
	...
	for (int i = 0; i < numPred; i++) {
		...
		getline(paramMonde, entreInfo); Manip.Tokenize(entreInfo, words); //-- Position
		//need to decide the positioning ourselves... not match the other EcoSim's positioning
		p.x = rand.next(475, 525);
		p.y = rand.next(475, 525);  words.clear();
		getline(paramMonde, entreInfo); Manip.Tokenize(entreInfo, words); //-- Energy
		e = (float)atof(words[1].c_str());  words.clear();
		getline(paramMonde, entreInfo); Manip.Tokenize(entreInfo, words);
		...
		Predator * newPred = new Predator(t, maxPredID, spec, p1id, p2id, p1sid, p2sid, p, e, eM, spd, sM, a, aM, v, rA, dE, sB, &fcm_pred, genderVal, 1, persuasionVal); //Add Gender /MRE/
		//cout << Manip.vtos(t) << "," << Manip.vtos(maxPredID) << "," << Manip.vtos(spec) << ","  << Manip.vtos(e) << "," << Manip.vtos(eM) << "," << Manip.vtos(spd) << "," << Manip.vtos(sM) << "," << Manip.vtos(a) << "," << Manip.vtos(aM) << "," << Manip.vtos(v) << "," << Manip.vtos(rA) << "," << Manip.vtos(dE) << "," << Manip.vtos(sB) << "," << Manip.vtos(genderVal) << endl;
		//cout.flush();
		...
		for (speciesIndex = 0; speciesIndex < predSpeciesFound.size(); speciesIndex++)
		{
			if (predSpeciesFound[speciesIndex] == spec) //found a species
			{
				foundTheSpecies = true;
				break;
			} 
		...
		{
			bool allocated = false;
			int speciesID = predSpeciesMapped[speciesIndex];
			for (list<Species>::iterator S = speciesPred.begin(); S != speciesPred.end(); ++S) //find the species
			{
				if ((*S).getNum() == speciesID) //found it
				{
					(*S).predMembers.push_back(&(wolves.back())); //add this pred to predMembers of that species
					(wolves.back()).setSpecies((*S).getNum()); //set the species of the pred to the correct species
					(wolves.back()).speciesPtr = &(*S); //update its pointer
					(*S).incNbMembers();
					allocated = true;
					break; //no sense in continuing
				}
			...
			if (allocated == false)
			{
				printf("\ndidn't find the pred species %d somehow though we determined that it exists as %d... need to re-create it\n", spec, speciesID);

				cout << "\nThe invasive pred species " << spec << " hasn't been made yet in this instance though it existed before... creating new species with ID " << speciesID << endl; cout.flush();
				FCMPredator s_fcm_pred;
				s_fcm_pred = FCMPredator((wolves.back()).getFCM()->get_chart(), NULL, &(wolves.back()));
				PhysicalGenome* genome_temp = (wolves.back()).getPhGenome();	// M.M
				//PhysicalGenome s_genome_prey = PhysicalGenome (Prey::nbGenes, genome_temp->getGenome(), probaMutation); // M.M
				PhysicalGenome s_genome_pred = PhysicalGenome(Predator::nbGenes, genome_temp->getRealGenome()); // M.M
				Species * newPredSpecies = new Species(speciesID, 0, &s_fcm_pred, &s_genome_pred); // M.M modified
				speciesPred.push_back(*newPredSpecies);
				speciesPred.back().predMembers.clear();
				speciesPred.back().predMembers.push_back(&(wolves.back()));
				wolves.back().speciesPtr = &(speciesPred.back());
				delete newPredSpecies;
				newSpeciesCreated++;
				//predSpeciesFound.push_back(spec);
				//predSpeciesMapped.push_back(Species::numSpeciesPredMax);
				speciesPred.back().incNbMembers();
				speciesPred.back().setParent(-1);
			}
		}
		else
		{
			Species::numSpeciesPredMax++;
			cout << "\nThe invasive pred species " << spec << " hasn't been made yet... creating new species with ID " << Species::numSpeciesPredMax << endl; cout.flush();
			...
			PhysicalGenome s_genome_pred = PhysicalGenome(Predator::nbGenes, genome_temp->getRealGenome()); // M.M
			Species * newPredSpecies = new Species(Species::numSpeciesPredMax, 0, &s_fcm_pred, &s_genome_pred); // M.M modified
			speciesPred.push_back(*newPredSpecies);
			speciesPred.back().predMembers.clear();
			speciesPred.back().predMembers.push_back(&(wolves.back()));
			wolves.back().speciesPtr = &(speciesPred.back());
			...
			speciesPred.back().setParent(-1);
			//create a new species and add this individual to that
				//add a value to PredSpeciesFound and its corresponding species ID in predSpeciesMapped
		}

		delete newPred;
		
	}
	cout << "   " << numPred << " pred, " << newSpeciesCreated << " invasive species created." << endl;
	
	*/
	//THIS I NEED TO DO: this recalculates species average genomes
	
	/*
	* Recalculate maps for species
	*/
	
	cout << "\nrearranging pointers first..." << endl; cout.flush();
	//Prey
	for (list<Species>::iterator S = speciesPrey.begin(); S != speciesPrey.end(); ++S) {
		(*S).preyMembers.clear();
		(*S).setNbMembers(0);
	...
	//-- 1. All pred in worldCreature are cleared
	for (int i = 0; i < width; ++i) {
		for (int j = 0; j < height; ++j) {
			worldPredArray[i][j].clear(); 
	...
	//-- 2. All pred are added back to worldCreature
	//int cntr = 0;
	for (size_t i = 0; i < wolves.size(); ++i) {
		addListCreature(*(wolves[i].getPosition()), 1, (int)i);
		if (wolves[i].getGender() == 0){ //MALE
			addListMaleCreature(*(wolves[i].getPosition()), 1, (int)i);
			//cntr++;
		}
	}
	...
	for (list<Species>::iterator it = speciesPrey.begin(); it != speciesPrey.end(); ++it) {
		...
		//cout << "\ncheck 5.5" << " " << (*it).preyMembers.size() << " " << (*it).getNum() << endl; cout.flush();
		
		for (size_t p = 0; p < (*it).preyMembers.size(); ++p) {	//-- For each member of this species
			//cout << "\ncheck 5.5.1" << " " << fAvg->get_rows() << " " << fAvg->get_cols(); cout.flush();
			
			//cout << "\n can we access preyMembers? " << (*it).preyMembers[p]->getID() << endl; cout.flush();
					
			
			for (size_t j = 0; j < fAvg->get_rows(); ++j) {
				for (size_t k = 0; k < fAvg->get_cols(); ++k) {
					//cout << "\ncheck 5.5.1.1" << " " << (*it).preyMembers[p]->getFCM()->get_chart(j, k); cout.flush();
			
					fcm_temp_value[j][k] += (*it).preyMembers[p]->getFCM()->get_chart(j, k);
					//cout << "\ncheck 5.5.1.2" << "cashmeoussidehowbouda" << endl; cout.flush();
			
				}
			}
			
			//cout << "\ncheck 5.5.2" << " " << (*it).preyMembers.size() << " " << (*it).getNum() << endl; cout.flush();
...
void Ecosystem::SpeciesStatistics() {
	...
	if (generation == 1){
		...
		statWorld.setSpeciesRatioInvPrey(0);
		statWorld.setSpeciesRatioInvPred(0);
		statWorld.setExtinctionRatioInvPrey(0);
		statWorld.setExtinctionRatioInvPred(0);
	}
	else{
		...
		statWorld.setSpeciesRatioInvPrey((float)nbNewSpeciesPrey / (float)(speciesPrey.size() - nbNewSpeciesPrey + nbExtinctSpeciesPrey));
		statWorld.setSpeciesRatioInvPred((float)nbNewSpeciesPred / (float)(speciesPred.size() - nbNewSpeciesPred + nbExtinctSpeciesPred));
		statWorld.setExtinctionRatioInvPrey((float)nbExtinctSpeciesPrey / (float)(speciesPrey.size() - nbNewSpeciesPrey + nbExtinctSpeciesPrey));
		statWorld.setExtinctionRatioInvPred((float)nbExtinctSpeciesPred / (float)(speciesPred.size() - nbNewSpeciesPred + nbExtinctSpeciesPred));
	}
}
...
void Ecosystem::restoreState() {
	...
	*/
	...
	for (int i = 0; i < numPrey; i++) {
		...
		getline(paramMonde, entreInfo); Manip.Tokenize(entreInfo, words);
		persuasionVal = (short)atoi(words[1].c_str()); words.clear();
		if (isTransferRun == 0){ //if not a transfer run, the run has done some computing (and hopefully MaxSaved) with the isInvader flag
			getline(paramMonde, entreInfo); Manip.Tokenize(entreInfo, words);
			isInvader = (short)atoi(words[1].c_str()); words.clear();
		}
		else isInvader = 0;		//if it is a transfer run, the run has been computing without the isInvader flag and there are no invaders in the MaxSaves yet
		getline(paramMonde, entreInfo); words.clear(); //-- Blank
		...
		*/
		...
		Prey * newPrey = new Prey(t, pid, spec, p1id, p2id, p1sid, p2sid, p, e, eM, spd, sM, a, aM, v, rA, dE, sB, &fcm_prey, genderVal, isInvader, d, cd, persuasionVal);	//Add Gender /MRE/
	...
	*/
	...
	for (int i = 0; i < numPred; i++) {
		...
		Predator * newPred = new Predator(t, pid, spec, p1id, p2id, p1sid, p2sid, p, e, eM, spd, sM, a, aM, v, rA, dE, sB, &fcm_pred, genderVal, isInvader, persuasionVal); //Add Gender /MRE/
	...
	*/
	...
	//Restore Invader Species Mapping
	cout << "Restoring invasive species mappings..." << endl;
	//Prey Species Found
	getline(paramMonde, entreInfo);
	getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);
	int numberOfInvPreySpecies = atoi(words[1].c_str());
	for (int nP = 1; nP <= numberOfInvPreySpecies; nP++) {
		getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);
		int currPreySpecID = atoi(words[0].c_str());
		preySpeciesFound.push_back(currPreySpecID);
		cout << "	-found prey species " << currPreySpecID << endl;
	}

	//Pred Species Found
	getline(paramMonde, entreInfo);
	getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);
	int numberOfInvPredSpecies = atoi(words[1].c_str());
	for (int nP = 1; nP <= numberOfInvPredSpecies; nP++) {
		getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);
		int currPredSpecID = atoi(words[0].c_str());
		predSpeciesFound.push_back(currPredSpecID);
		cout << "	-found pred species " << currPredSpecID << endl;
	}

	//Prey Species Mapped
	getline(paramMonde, entreInfo);
	getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);
	int numberOfInvPreySpeciesMapped = atoi(words[1].c_str());
	for (int nP = 1; nP <= numberOfInvPreySpeciesMapped; nP++) {
		getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);
		int currPreySpecID = atoi(words[0].c_str());
		preySpeciesMapped.push_back(currPreySpecID);
		cout << "	-mapped prey species " << currPreySpecID << endl;
	}

	//Pred Species Mapped
	getline(paramMonde, entreInfo);
	getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);
	int numberOfInvPredSpeciesMapped = atoi(words[1].c_str());
	for (int nP = 1; nP <= numberOfInvPredSpeciesMapped; nP++) {
		getline(paramMonde, entreInfo); words.clear(); Manip.Tokenize(entreInfo, words);
		int currPredSpecID = atoi(words[0].c_str());
		predSpeciesMapped.push_back(currPredSpecID);
		cout << "	-mapped pred species " << currPredSpecID << endl;
	}
...
void Ecosystem::TwoMeanPreySpeciation() {
	...
	*/
	...
	for (list<Species>::iterator S = speciesPrey.begin(); S != speciesPrey.end(); ++S) {
		...
		if (fDistance > 1.4 * distanceSpeciesPrey && (*S).getNum() != -2) {
...
void Ecosystem::initializeMaxGrass(){
	if (circularFoodGrowth == 1){
		...
		for (a = 0; a < width; a++){
			for (b = 0; b < height; b++){
				// divided the first quadrant into 4 circles
				if (a < 500 && b < 500)
					worldMaxGrassArray[a][b] = (float)fmin(fmax((sin(pi*a/250) * sin(pi*b/250)) * (foodCircleMaxRatio) + foodCircleMinimumRatio, -(sin(pi*a/250) * sin(pi*b/250)) * (foodCircleMaxRatio) + foodCircleMinimumRatio), 1.0);
				// by decreasing ratios, dropoff on the second quadrant becomes more severe
				if (a >= 500 && b < 500)
					worldMaxGrassArray[a][b] = (float)fmin(fmax((sin(pi*a/foodCircleDiameter) * sin(pi*b/foodCircleDiameter)) * (foodCircleMaxRatio-0.4) + (foodCircleMinimumRatio-0.4), -(sin(pi*a/foodCircleDiameter) * sin(pi*b/foodCircleDiameter)) * (foodCircleMaxRatio-0.4) + (foodCircleMinimumRatio-0.4)), 1.0);
				// third quadrant is as normal
				if (a < 500 && b >= 500)
				...
				// fourth quadrant can only go up to half the max
				if (a >= 500 && b >= 500)
					worldMaxGrassArray[a][b] = 0.5 * (float)fmin(fmax((sin(pi*a/foodCircleDiameter) * sin(pi*b/foodCircleDiameter)) * (foodCircleMaxRatio) + foodCircleMinimumRatio, -(sin(pi*a/foodCircleDiameter) * sin(pi*b/foodCircleDiameter)) * (foodCircleMaxRatio) + foodCircleMinimumRatio), 1.0);
			}
		}
	}
	else{
...
void Ecosystem::snap(){
	...
	for(int i=0;i<1000;i++)
	{
		for(int j=0;j<1000;j++)
		{
			...
			int invPreyCount = 0; int invPredCount = 0;
			
			vector<int> rabbitList = *(getListCreature(p, 0));
			vector<int> wolfList = *(getListCreature(p, 1));
			for (int b = 0; b < rabbitList.size(); b++)
			{
				if (rabbits[rabbitList[b]].getIsInvasive() == 1) invPreyCount++;
			}
			
			for (int b = 0; b < wolfList.size(); b++)
			{
				if (wolves[wolfList[b]].getIsInvasive() == 1) invPredCount++;
			}
			
			//cout << "At " << p.x << "," << p.y << ": " << invPreyCount << " " << invPredCount << " " << cnt << " " << cnt2 << endl; cout.flush();
			cnt -= invPreyCount;
			...
			if(cnt2>10)
				cnt2=10;
			if (invPreyCount>15) invPreyCount = 15;
			if (invPredCount>10) invPredCount = 10;
			// Plotting Prey
			while (cnt!=0)
			{
				Position pos;
			...
			while (invPreyCount!=0)
			{
				...
				while(temp.Red!=0 && temp.Green!=0);
				snapShot.SetPixel(pos.y,pos.x,invPreyPix);
				invPreyCount--;
			...
			while (invPredCount!=0)
			{
				...
				while(temp.Red!=0 && temp.Green!=0);
				snapShot.SetPixel(pos.y,pos.x,invPredPix);
				invPredCount--;
