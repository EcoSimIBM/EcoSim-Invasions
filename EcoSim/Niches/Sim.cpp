...
Sim::Sim(short type, long ID, int species, long parent1ID, long parent2ID, int speciesParent1ID, int speciesParent2ID, float distanceEvol, unsigned char nbGenes, short gender, short isInvasive){//Add Gender /MRE/
...
float Sim::calculationEnergyAction(float energy, int nbArc, float speed, float strength, float persuasion, Ecosystem * eco){
	...
	if (phGenome.getGenomeSize() > 6){ // prey
		...
		if (getGender() == 0){
			if (this->getIsInvasive() == 0) eco->statWorld.incEnergySpentMalePrey(energyToSpend);
			else eco->statWorld.incEnergySpentInvMalePrey(energyToSpend);
		}
		else if (getGender() == 1){
			if (this->getIsInvasive() == 0) eco->statWorld.incEnergySpentFemalePrey(energyToSpend);
			else eco->statWorld.incEnergySpentInvFemalePrey(energyToSpend);
		}
		...
		e = energy - energyToSpend;
		if (getGender() == 0){
			if (this->getIsInvasive() == 0) eco->statWorld.incEnergySpentMalePred(energyToSpend);
			else eco->statWorld.incEnergySpentInvMalePred(energyToSpend);
		}
		else if (getGender() == 1){
			if (this->getIsInvasive() == 0) eco->statWorld.incEnergySpentFemalePred(energyToSpend);
			else eco->statWorld.incEnergySpentInvFemalePred(energyToSpend);
		}
