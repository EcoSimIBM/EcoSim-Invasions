...
public:
	...
	vector<int> preySpeciesFound;
	vector<int> preySpeciesMapped;
	vector<int> predSpeciesFound;
	vector<int> predSpeciesMapped;
	...
	//Invader Stuff
	int isTransferRun;		// is this run supposed to start normally but become an invader run? 1 = true, 0 = false
	int isAcceptor;			// is this run accepting invaders? 1 = true, 0 = false
	int isDonator;			// is this run donating invaders? 1 = true, 0 = false
	int numberPreyInvaders;		// how many prey invaders are we sending?
	int numberPredInvaders;		// how many pred invaders are we sending?
	int invasionFrequency;  // how often are we saving/loading invaders?
	int clonesOnly;			// are we only saving a clone or are we saving a varied population?
	
	float maxModifiedGrass;

	RGBApixel preyPix, predPix, invPreyPix, invPredPix, grassPix, meatPix, bkground; 
	BMP snapShot;
	BMP snapShot2;
	BMP snapShot3;
	...
	int getNbLocalSameType(Position, int, int);
